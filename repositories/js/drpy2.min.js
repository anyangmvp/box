...
function maoss(jxurl, ref, key) {
	...
	try {
		...
		if (ref) {
			var html = request(jxurl, { headers: { Referer: ref } });
		} else {
		...
		if (!config.url.startsWith('http')) {
			config.url = CryptoJS.AES.decrypt(config.url, key, { iv: iv, padding: CryptoJS.pad.Pkcs7 }).toString(
				CryptoJS.enc.Utf8
			);
		}
		return config.url;
...
function searchParse(searchObj) {
	...
	return JSON.stringify({ page: parseInt(searchObj.pg), pagecount: 10, limit: 20, total: 100, list: d });
}
			...
			if (p.lists.startsWith('js:')) {
				...
				for (let i = 0; i < playFrom.length; i++) {
					...
					if (new_vod_list.length > 0) {
						...
						console.log(
							`drpy影响性能代码共计列表数循环次数:${new_vod_list.length},耗时:${
...
function playParse(playObj) {
	...
	var input = MY_URL;
	let common_play = { parse: 1, url: input, jx: tellIsJx(input) };
	let lazy_play;
	if (!rule.play_parse || !rule.lazy) {
		...
		try {
			...
			eval(lazy_code);
			lazy_play = typeof input === 'object' ? input : { parse: 1, jx: tellIsJx(input), url: input };
		} catch (e) {
	...
	if (Array.isArray(rule.play_json) && rule.play_json.length > 0) {
		...
		let base_json = { jx: 1, parse: 1 };
		lazy_play = Object.assign(lazy_play, base_json);
	} else if (!rule.play_json) {
		let base_json = { jx: 0, parse: 1 };
		lazy_play = Object.assign(lazy_play, base_json);
	}
...
function homeVod(params) {
	console.log('homeVod');
	let homeVodObj = { 推荐: rule.推荐, double: rule.double, homeUrl: rule.homeUrl, detailUrl: rule.detailUrl };
	return homeVodParse(homeVodObj);
}
function category(tid, pg, filter, extend) {
	let cateObj = { url: rule.url, 一级: rule.一级, tid: tid, pg: parseInt(pg), filter: filter, extend: extend };
...
function play(flag, id, flags) {
	let playObj = { url: id, flag: flag, flags: flags };
...
function search(wd, quick, pg) {
	...
	let searchObj = { searchUrl: rule.searchUrl, 搜索: rule.搜索, wd: wd, pg: pg || 1, quick: quick };
...
function proxy(params) {
	...
	let proxyObj = { params: params, proxy_rule: rule.proxy_rule };
...
function isVideo(url) {
	...
	let isVideoObj = { url: url, isVideo: is_video, t: t };
